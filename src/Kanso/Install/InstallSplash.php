<html lang="en-US" xmlns="http://www.w3.org/1999/xhtml">

<head>
    
    <!-- HTML -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
    <title>Welcome To Kanso</title>

    <!-- FAVICONS -->
    <link rel="shortcut icon" href="<?php echo $adminAssetsPth; ?>images/favicon.png">
    <link rel="apple-touch-icon" sizes="57x57" href="<?php echo $adminAssetsPth; ?>images/apple-touch-icon.png">
    <link rel="apple-touch-icon" sizes="72x72" href="<?php echo $adminAssetsPth; ?>images/apple-touch-icon-72x72.png">
    <link rel="apple-touch-icon" sizes="114x114" href="<?php echo $adminAssetsPth; ?>images/apple-touch-icon-114x114.png">

</head>
<body style="-moz-box-sizing: border-box;box-sizing: border-box;padding: 0;margin: 0;border: 0;font: inherit;vertical-align: baseline;line-height: 1;height: 100%;background: #e5e5e5;font-family: &quot;Helvetica Neue&quot;,Helvetica,&quot;Segoe UI&quot;,Arial,freesans,sans-serif;color: #1e2629;-webkit-font-smoothing: antialiased;-moz-osx-font-smoothing: grayscale;">

<svg width="512" height="512" viewbox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="display: none;-moz-box-sizing: border-box;box-sizing: border-box;width: 14px;height: 14px;">
    <g id="logo"><path fill="#62C4B6" d="M297.011,22.469c0,9.491-7.765,17.256-17.256,17.256H153.851c-9.491,0-17.255-7.765-17.255-17.256v-2.551c0-9.491,7.764-17.256,17.255-17.256h125.904c9.491,0,17.256,7.765,17.256,17.256V22.469z"/><path fill="#62C4B6" d="M404.228,88.021c0,9.491-7.766,17.256-17.256,17.256H110.73c-9.491,0-17.255-7.765-17.255-17.256v-2.55c0-9.492,7.764-17.257,17.255-17.257h276.242c9.49,0,17.256,7.765,17.256,17.257V88.021z"/><path fill="#62C4B6" d="M460.526,153.573c0,9.491-7.767,17.255-17.256,17.255H112.456c-9.492,0-17.256-7.764-17.256-17.255v-2.551c0-9.492,7.764-17.256,17.256-17.256H443.27c9.489,0,17.256,7.765,17.256,17.256V153.573z"/><path fill="#35495E" d="M404.228,211.757c0,9.489-7.766,17.256-17.256,17.256H276.566c-9.491,0-17.257-7.768-17.257-17.256v-2.554c0-9.49,7.766-17.254,17.257-17.254h110.406c9.49,0,17.256,7.764,17.256,17.254V211.757z"/><path fill="#35495E" d="M434.91,267.817c0,9.492-7.766,17.259-17.255,17.259h-78.968c-9.488,0-17.254-7.767-17.254-17.259v-2.548c0-9.49,7.766-17.255,17.254-17.255h78.968c9.489,0,17.255,7.765,17.255,17.255V267.817z"/><path fill="#35495E" d="M425.146,323.885c0,9.493-7.767,17.259-17.257,17.259H186.136c-9.491,0-17.255-7.766-17.255-17.259v-2.555c0-9.488,7.765-17.254,17.255-17.254h221.753c9.49,0,17.257,7.766,17.257,17.254V323.885z"/><path fill="#F5B54C" d="M410.804,379.95c0,9.488-7.766,17.259-17.255,17.259H112.454c-9.491,0-17.256-7.771-17.256-17.259v-2.552c0-9.488,7.766-17.255,17.256-17.255h281.095c9.489,0,17.255,7.767,17.255,17.255V379.95z"/><path fill="#FFD061" d="M381.722,436.018c0,9.488-7.767,17.255-17.256,17.255H141.538c-9.491,0-17.254-7.767-17.254-17.255v-2.556c0-9.491,7.764-17.257,17.254-17.257h222.928c9.489,0,17.256,7.766,17.256,17.257V436.018z"/><path fill="#FFD061" d="M340.821,492.08c0,9.491-7.765,17.258-17.255,17.258H182.44c-9.49,0-17.254-7.767-17.254-17.258v-2.553c0-9.488,7.764-17.254,17.254-17.254h141.125c9.49,0,17.255,7.766,17.255,17.254V492.08z"/><path fill="#62C4B6" d="M234.892,219.125c0,9.491-7.766,17.257-17.257,17.257H115.601c-9.491,0-17.255-7.766-17.255-17.257v-2.551c0-9.49,7.764-17.256,17.255-17.256h102.034c9.491,0,17.257,7.766,17.257,17.256V219.125z"/></g>
</svg>

<div id="wrapper" style="-moz-box-sizing: border-box;box-sizing: border-box;margin: 0;padding: 0;border: 0;font: inherit;vertical-align: baseline;position: relative;min-height: 100%;">

    <div class="site-container cleafix login setup-panel" style="-moz-box-sizing: border-box;box-sizing: border-box;margin: 0;padding: 40px 0;border: 0;font: inherit;vertical-align: baseline;position: absolute;top: 50%;left: 50%;max-width: 700px;min-width: 700px;margin-right: auto;margin-left: -350px;width: 700px;margin-top: -200px;padding-left: 10px;padding-right: 10px;">
        <div class="panel" style="-moz-box-sizing: border-box;box-sizing: border-box;margin: 0 auto;padding: 20px;border: 1px solid #d8d8d8;font: inherit;vertical-align: baseline;border-radius: 3px;background-color: #fff;border-bottom-width: 2px;">
            
            <div class="logo-wrap" style="-moz-box-sizing: border-box;box-sizing: border-box;margin: 0 auto;padding: 0;border: 0;font: inherit;vertical-align: baseline;text-align: center;margin-bottom: 20px;">
                <svg viewbox="0 0 512 512" class="icon" style="-moz-box-sizing: border-box;box-sizing: border-box;width: 50px;height: 50px;opacity: 0.4;"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#logo" style="-moz-box-sizing: border-box;box-sizing: border-box;"></use></svg>
            </div>

            <h6 style="margin-bottom: 10px;-moz-box-sizing: border-box;box-sizing: border-box;margin: 0;padding: 0;border: 0;font: inherit;vertical-align: baseline;font-weight: 900;font-size: 25px;letter-spacing: -.08rem;">Welcome</h6>

            <p style="margin: 20px 0;line-height: 27px !important;-moz-box-sizing: border-box;box-sizing: border-box;padding: 0;border: 0;font: inherit;vertical-align: baseline;">
            Kanso was succefully installed on your server! 
            You can access your administrator panel by logging in <a href="/admin/login/" style="-moz-box-sizing: border-box;box-sizing: border-box;text-decoration: underline;color: #62c4b6;margin: 0;padding: 0;border: 0;font: inherit;vertical-align: baseline;outline: 0;">here</a> and using the username and password you entered in your installation settings file.
            </p>

            <a class="button" href="/admin/login/" style="-moz-box-sizing: border-box;box-sizing: border-box;text-decoration: none;color: #1e2629;margin: 0;padding: 7px 18px;border: 1px solid #BDC3CA;font: inherit;vertical-align: baseline;outline: 0;font-weight: 900;font-size: 13px;position: relative;border-radius: 2px;-moz-transition: .2s ease;-o-transition: .2s ease;-webkit-transition: .2s ease;transition: .2s ease;line-height: 29px;background-color: #FAFBFB;box-shadow: 0 1px 0 rgba(153,156,160,.17);">Login</a>
            
        </div>
    </div>

</div></body>
</html>