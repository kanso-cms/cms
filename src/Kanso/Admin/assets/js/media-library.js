(function() {
    ...
    MediaLibrary.prototype._submitBulkDelete = function() {
        ...
        Ajax.post(ajaxURL, form, function(success) {
            ...
            if (responseObj && responseObj.details && responseObj.details === 'valid') {
    ...
    MediaLibrary.prototype._submitUpdateInfo = function() {
        var form = {
            'ajax_request' : 'update_image_info',
    ...
    MediaLibrary.prototype._requestImages = function() {
        ...
        Ajax.post(ajaxURL, this._imageForm(), function(success) {
            ...
            if (responseObj && responseObj.details) {
                self._handleResponse(responseObj.details);
                return;
            }
            self._setAsEmpty();
        },
    ...
    MediaLibrary.prototype._initDzEvents = function(DZ) {
        ...
        DZ.on("addedfile", function(file) {
            ...
            Ajax.upload(ajaxURL, form, 
                function(success) {
                    var response = Helper.isJSON(success);
                    if (response && response.details) {
                        if (Helper.is_array(response.details) && Helper.isset(response.details[0])) {
                            updateUploadedItem(response.details[0], node);
                            resetProgress();
                            return;
                        }
                    }
